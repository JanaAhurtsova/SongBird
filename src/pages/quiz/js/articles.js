export class Article {
  ...
  buildArticle() {
    ...
    this.birdsSpecies =this.createDomNode(this.birdsSpecies, 'div', 'bird-species');
    ...
    this.playBtn = this.createDomNode(this.playBtn, 'button', 'play', 'player__icon');

    this.progressBar = this.createDomNode(this.progressBar, 'div', 'progress-bar');
    ...
    this.playerTimes = this.createDomNode(this.playerTimes, 'div', 'player-time');

    this.currentTime = this.createDomNode(this.currentTime, 'div', 'timer');
    this.currentTime.innerHTML = '00:00';

    this.duration = this.createDomNode(this.duration, 'div', 'duration');
    this.duration.innerHTML = '00:00';

    this.descriptionCard = this.createDomNode(this.description, 'p','description');
  ...
  showCard() {
    document.querySelector('.correct-answer').append(this.article);
  }

  bindEvents() {
    this.playBtn.addEventListener('click', this.playAudio.bind(this));
    this.audioCard.addEventListener('timeupdate', this.updateProgress.bind(this));
  ...
  setPositionSeekbar(e) {
    ...
    this.audioCard.currentTime = (clickedOffsetX / progressWidth) * this.audioCard.duration;
    this.audioCard.play();
  }
  ...
  updateProgress() {
    ...
    if(!isNaN(this.audioCard.duration)) {
      ...
      this.currentTime.innerHTML = this.formatTime(Math.floor(this.audioCard.currentTime));
      this.duration.innerHTML = this.formatTime(Math.floor(this.audioCard.duration));
    }
  }
